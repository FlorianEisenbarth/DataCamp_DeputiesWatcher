...
def get_train_data():
    ...
    y = df["position"]
    X = df[["parti", "vote_objet", "vote_demandeur", "vote_date", "vote_uid"]]
...
preprocessing = make_pipeline(
    SimpleImputer(strategy="constant", fill_value="unknow"), OneHotEncoder()
)

# simple vectorizer encoding (All the challenge is here !!)
vote_objet_encoding = make_pipeline(CountVectorizer(), TfidfTransformer())

transform = make_column_transformer(
    (preprocessing, ["vote_demandeur", "demandeur_parti"]),
...
cm_df = pd.DataFrame(
    confusion_matrix(y_test, y_pred), columns=["N", "P"], index=["N", "P"]
)
sns.heatmap(
    cm_df,
    annot=True,
